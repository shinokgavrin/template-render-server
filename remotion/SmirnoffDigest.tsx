...
export const SmirnoffDigest: React.FC<{
    ...
    // 🔥 ПРОФЕССИОНАЛЬНАЯ ОБРАБОТКА ЗВУКА (Устранение щелчков) 🔥
    // Вместо резкого 1/0 используем interpolate для создания микро-фейдов по 3 кадра
    const currentVolume = actions?.reduce((acc, action) => {
        if (action.type === 'mute_title' || action.type === 'mute') {
            ...
            // Плавное затухание (fade-out) за 3 кадра до старта
            const fadeOut = interpolate(frame, [startFrame - 3, startFrame], [1, 0], { 
                extrapolateLeft: 'clamp', 
                extrapolateRight: 'clamp' 
            });
            
            // Плавное нарастание (fade-in) за 3 кадра после конца
            const fadeIn = interpolate(frame, [endFrame, endFrame + 3], [0, 1], { 
                extrapolateLeft: 'clamp', 
                extrapolateRight: 'clamp' 
            });
            
            // Если мы находимся в зоне действия этого "мьюта", применяем его кривую громкости
    ...
    return (
        <AbsoluteFill style={{ backgroundColor: 'black' }}>
            ...
            {/* === 2. ГИБКОЕ АУДИО (С защитой от цифровых щелчков) === */}
