...
export const SmirnoffDigest: React.FC<{
	...
	return (
		<AbsoluteFill style={{ backgroundColor: 'black' }}>
			...
			{/* === 1. ОСНОВНОЕ ВИДЕО (Возвращен стандартный Video для радикальной экономии RAM) === */}
							...
							<AbsoluteFill style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', pointerEvents: 'none' }}>
								{isVideoAsset ? (
									<Loop durationInFrames={durationInFrames}>
										<AbsoluteFill style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', pointerEvents: 'none' }}>
											{/* Для коротких реакций оставляем OffthreadVideo — здесь он безопасен и дает идеальную точность кадров */}
											<OffthreadVideo 
												src={action.url} 
												style={{ 
													width: '70%',       
